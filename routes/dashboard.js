...
router.get('/', function (req,res) {
    ...
    if(req.signedCookies['userId']){
        ...
        request(groupsRequest,function (error, groupResponse, groupBody) {
            if (error) res.render('error',{layout:'layout', error});
            ...
            if(groupResponse.statusCode!==200){
                ...
                if(req.query.source === 'upload'){
                    res.render('dashboard' ,{layout:'layout',success:true, successMessage:"Image uploaded successfully", data:groupBody.groups});
                }else if(req.query.source === 'resetpassword'){
                //custom message for successful password update
                    res.render('dashboard' ,{layout:'layout',success:true, successMessage:"Updated password", data:groupBody.groups});
                }else if(req.query.source === 'eupload'){
                    res.render('dashboard' ,{layout:'layout',error:true, errorMessage:"something went wrong", data:groupBody.groups});
                }else{
                //if no success or from login route it to dashboard without headers
                    res.render('dashboard', {layout:'layout', home:false, data:groupBody.groups})
                }
            }
        })
    }else{
    ...
    if(req.signedCookies['userId']) {
        var groupsRequest = { method: 'POST',
            ...
            headers:
                {'cache-control': 'no-cache',
                    'Content-Type': 'application/json' },
            body: { userId:parseInt(req.signedCookies['userId'],10), password:req.body.newPassword  },
            json: true };
        request(groupsRequest,function (error, groupResponse, groupBody) {
            if(groupResponse.statusCode!==200){
